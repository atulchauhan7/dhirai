...
function ssw(){gapPx=parseFloat(getComputedStyle(track).columnGap)||0;const w=gsw();slides.forEach(s=>{s.style.width=w+'px';s.style.flexShrink='0'});if(center){const aw=slides[0]&&slides[0].offsetWidth>0?slides[0].offsetWidth:w;const vpW=vp.offsetWidth,actualTrackW=slides.length*aw+(slides.length-1)*gapPx;if(actualTrackW>vpW){const pk=(vpW-aw)/2;track.style.paddingLeft=pk+'px';track.style.paddingRight=pk+'px';}else{track.style.paddingLeft='';track.style.paddingRight='';}}else{track.style.paddingLeft='';track.style.paddingRight='';}}
...
function dEnd(){if(!dragging)return;dragging=false;track.style.cursor='';if(isDragged){justDragged=true;setTimeout(function(){justDragged=false},300);var mv=curTr-prevTr;if(Math.abs(mv)>gaw()/4){if(mv<0)next();else prev()}else go(idx)}isDragged=false;startAP()}
